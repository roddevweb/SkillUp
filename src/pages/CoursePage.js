...
const CoursePage = ({ courseId = 1 }) => {
    ...
    useEffect(() => {
        fetch(`/api/cours/${courseId}`)
            ...
            .catch(err => console.error('Erreur de chargement du cours', err));
    }, [courseId]);

    if (!course) {
        return <div style={{ padding: 40, textAlign: 'center' }}>Chargement du cours...</div>;
    }

    const etapes = course.etapes || [];
    ...
    const handleOptionClick = (idx) => {
        ...
        setIsCorrect(currentEtape.quiz && idx === currentEtape.quiz.answer);
    };
    ...
    return (
        <div style={{ maxWidth: 700, margin: '40px auto', background: '#fff', borderRadius: 12, boxShadow: '0 4px 24px rgba(0,0,0,0.08)', padding: '2.5rem 2rem' }}>
            <h2 style={{ textAlign: 'center', marginBottom: 24 }}>
                Cours de {course.langue} - Niveau {course.niveau}
            </h2>
                        ...
                        <>
                                    ...
                                    <button
                                        ...
                                        style={{
                                            ...
                                            background: selectedOption === idx
                                                ? (isCorrect && idx === currentEtape.quiz.answer ? '#d4edda' : '#f8d7da')
                                                : '#fff',
                                            color: '#232323',
                                            fontWeight: 500,
                                            fontSize: 16,
                            ...
                            {selectedOption !== null && (
                                <div style={{ marginBottom: 16, color: isCorrect ? '#388e3c' : '#d32f2f', fontWeight: 500 }}>
                                    ...
                                    <div style={{ color: '#555', fontWeight: 400, marginTop: 4 }}>
                                        {currentEtape.quiz.explanation}
                                    </div>
                                </div>
                            )}
                            <button
